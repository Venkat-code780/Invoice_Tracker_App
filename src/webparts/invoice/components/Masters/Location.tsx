...
class LocationRe extends React.Component<LocationProps, LocationState>{
  ...
  private SubmitData = () => {
     showLoader();
    let data = {
      location: { val: this.state.Location.trim(), required: true, Name: "'Location'", Type: ControlType.string, Focusid: this.inputProgram },
  
    };
    let isValid = formValidation.checkValidations(data);
    var formdata = {
      Title: this.state.Location.trim(),
...
private insertorupdateListitem = async (formData: any, list: any) => {
  ...
  private onError = () => {
    this.setState({
      loading: false, modalTitle: 'Error', modalText: 'Sorry! something went wrong', showHideModal: false, isSuccess: false, ItemID: 0, errorMessage: ""
    });
  }
