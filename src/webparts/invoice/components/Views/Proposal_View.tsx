...
class ProposalView extends React.Component<ProposalViewProps, ProposalViewState> {
  public state = {
    data: [],
    allData: [],
    columns: [],
  ...
  //       sp.web.lists.getByTitle(TrList).items.expand("SubmittedBy").select("SubmittedBy/Title","SubmittedBy/Id","*").orderBy("Id", false).get().
  //         then((response: any[]) => {
  ...
  private BindData(response: any) {
    ...
    response.forEach((Item: any) => {
      data.push({
        ...
        Amount: Item.Amount,
        Status: Item.Status,
        SubmittedBy: Item.SubmittedBy != null ? Item.SubmittedBy.Title : '',
  ...
  public render() {
    let columns = [
      ...
      //                           <NavLink title="Edit" className="csrLink ms-draggable" to={`/Proposal/${record.Id}`}>
      ...
      //           {
      //             name: "Created By",
      //             selector: (row:any, i:any) => row.SubmittedBy,
      //             sortable: true,
      //           },
      //           {
      //             name: "Created Date",
  ...
  // }
...

      {
        ...
        valueGetter: (params: any) =>
          params.data.SubmittedBy?.Title || params.data.SubmittedBy || "-",
      },
